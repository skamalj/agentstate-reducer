...
!!! note "Current version"
    `langgraph-dynamodb-checkpoint` **0.5.0** · Requires **Python 3.10+**
...
!!! success "Passes LangGraph's official checkpointer conformance suite — FULL, all 8 capabilities (langgraph-dynamodb-checkpoint 0.5.0)"
    Validated with [`langgraph-checkpoint-conformance`](https://pypi.org/project/langgraph-checkpoint-conformance/), the suite LangGraph's docs name as the validation path. Base: `put`, `put_writes`, `get_tuple`, `list` (ordering, `before`, `limit`, metadata filters, namespaces, pending writes), `delete_thread`. Extended, new in 0.5.0: `copy_thread`, `delete_for_runs`, `prune`. The conformance test ships in the repo's `tests/` and asserts the FULL level.

### LangSmith Deployment

LangSmith Deployment probes a custom checkpointer for these capabilities at startup: without `delete_for_runs` the rollback multitask strategy is unavailable, without `copy_thread` thread forking falls back to a slow per-checkpoint copy, and without `prune` history pruning is off. `DynamoDBSaver` implements all three, so it is a drop-in through `langgraph.json`:
...
!!! warning "Two caveats"
    `prune` is not `DeltaChannel`-aware; do not use `keep_latest` on threads whose graph uses `DeltaChannel`. `delete_for_runs` matches a `run_id` attribute written since 0.5.0; checkpoints stored by earlier versions are not found.
