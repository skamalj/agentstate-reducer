...
!!! note "Current version"
    `langgraph-checkpoint-firestore` **0.4.0** · Requires **Python 3.10+**
...
!!! success "Passes LangGraph's official checkpointer conformance suite — FULL, all 8 capabilities (langgraph-checkpoint-firestore 0.4.0)"
...
LangSmith Deployment probes a custom checkpointer for these capabilities at startup: without `delete_for_runs` the rollback multitask strategy is unavailable, without `copy_thread` thread forking falls back to a slow per-checkpoint copy, and without `prune` history pruning is off. `FirestoreSaver` implements all three, so it is a drop-in through `langgraph.json`:
